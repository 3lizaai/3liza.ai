...
import torch
from eliza.model import *
from eliza.constants import DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    ...
    if 'eliza' in model_name.lower():
        # Load LLaVA model
        if 'lora' in model_name.lower() and model_base is None:
            warnings.warn('There is `lora` in model name but no `model_base` is provided. If you are loading a LoRA model, please provide the `model_base` argument. Detailed instruction: https://github.com/haotian-liu/LLaVA#launch-a-model-worker-lora-weights-unmerged.')
        if 'lora' in model_name.lower() and model_base is not None:
            from eliza.model.language_model.eliza_llama import ElizaConfig
            base_cfg_pretrained = ElizaConfig.from_pretrained(model_base)
            print('Loading Eliza from base model...')
            model = ElizaLlamaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=base_cfg_pretrained, ignore_mismatched_sizes=True, **kwargs)
            
            print('Initialize Eliza...')
            tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
           
            print('Loading additional Eliza weights...')
            ...
            tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
            from eliza.model.language_model.eliza_llama import ElizaConfig
            cfg_pretrained = ElizaConfig.from_pretrained(model_base)
            print('cfg_pretrained: ', cfg_pretrained)
            model = ElizaLlamaForCausalLM.from_pretrained(model_base, config=cfg_pretrained).to(device="cuda", dtype=torch.float16) 
            print("kwargs: ", kwargs)
            ...
            model = ElizaLlamaForCausalLM.from_pretrained(
                model_path,
